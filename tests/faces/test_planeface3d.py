...
class TestPlaneFace3D(unittest.TestCase):
    ...
    def test_face_inside(self):
        face2 = self.face.frame_mapping(
            volmdlr.Frame3D(
                volmdlr.Point3D(0, 0, 0),
                volmdlr.Vector3D(0.5, 0, 0),
                volmdlr.Vector3D(0, 0.5, 0),
                volmdlr.Vector3D(0, 0, 0.5),
            ),
            "old",
        )
    ...
    def test_line_intersections(self):
        line_inside_hole = edges.Line3D(volmdlr.Point3D(0.1, 0.0, -0.3), volmdlr.Point3D(-0.1, 0.0, 0.3))
        line_inside_face = edges.Line3D(volmdlr.Point3D(-0.05, 0.0, -0.3), volmdlr.Point3D(-0.1, 0.0, 0.3))
    ...
    def test_cylindricalface_intersections(self):
        ...
        face = cylindricalsurface.rectangular_cut(0, volmdlr.TWO_PI, -0.25, 0.25)
        ...
        self.assertEqual(
            face_intersections[0].primitives[0].major_dir, volmdlr.Vector3D(0, 0.7071067811865475, -0.7071067811865475)
        )
        """ ========== THREE ARC ELLIPSES ========="""
        ...
        face_intersections = plane_face_3.face_intersections(face)
        self.assertEqual(
            face_intersections[0].primitives[0],
            edges.LineSegment3D(volmdlr.Point3D(0.15, 0.0, -0.25), volmdlr.Point3D(0.15, 0.0, 0.25)),
        )
        self.assertEqual(
            face_intersections[1].primitives[0],
            edges.LineSegment3D(volmdlr.Point3D(-0.15, 0.0, -0.25), volmdlr.Point3D(-0.15, 0.0, 0.25)),
        )
        """ ========== ONE LINE ========="""
        ...
        self.assertEqual(
            face_intersections[0].primitives[0],
            edges.LineSegment3D(volmdlr.Point3D(0.0, 0.15, -0.25), volmdlr.Point3D(0.0, 0.15, 0.25)),
        )
