...
class TestCylindricalSurface3D(unittest.TestCase):
    ...
    def test_line_intersections(self):
        line3d = edges.Line3D(volmdlr.O3D, volmdlr.Point3D(0.3, 0.3, 0.3))
    ...
    def test_plane_intersections(self):
        ...
        parallel_plane_secant_cylinder = plane_surface.frame_mapping(
            volmdlr.Frame3D(
                volmdlr.O3D, volmdlr.Point3D(0.5, 0, 0), volmdlr.Point3D(0, 0.5, 0), volmdlr.Point3D(0, 0, 0.2)
            ),
            "new",
        )
        ...
        cylinder_surface_tangent_plane = self.cylindrical_surface.plane_intersection(cylinder_tanget_plane)
    ...
    def test_arc3d_to_2d(self):
        arc1 = edges.Arc3D(
            volmdlr.Point3D(1, 0, 0), volmdlr.Point3D(1 / math.sqrt(2), 1 / math.sqrt(2), 0), volmdlr.Point3D(0, 1, 0)
        )
        arc2 = edges.Arc3D(
            volmdlr.Point3D(1, 0, 0), volmdlr.Point3D(1 / math.sqrt(2), -1 / math.sqrt(2), 0), volmdlr.Point3D(0, -1, 0)
        )
        ...
        arc4 = edges.Arc3D(
            volmdlr.Point3D(0, -1, 0), volmdlr.Point3D(-1 / math.sqrt(2), 1 / math.sqrt(2), 0), volmdlr.Point3D(1, 0, 0)
        )
    ...
    def test_face_from_contours3d(self):
        surface = dessia_common.core.DessiaObject.load_from_file("faces/objects_cylindrical_tests/surface3d_1.json")
        contour0 = dessia_common.core.DessiaObject.load_from_file("faces/objects_cylindrical_tests/contour_1_0.json")
        contour1 = dessia_common.core.DessiaObject.load_from_file("faces/objects_cylindrical_tests/contour_1_1.json")
