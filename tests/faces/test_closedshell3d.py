...
class TestClosedShell3D(unittest.TestCase):
    def test_union_adjacent_blocks(self):
        ...
        contour = volmdlr.wires.ClosedPolygon2D(
            [
                volmdlr.Point2D(0, 0),
                volmdlr.Point2D(-1, 0),
                volmdlr.Point2D(-1, 1),
    ...
    def test_set_operations_blocks(self):
        box_red = primitives3d.Block(
            volmdlr.Frame3D(
                ...
                volmdlr.Vector3D(0.4, 0, 0),
                volmdlr.Vector3D(0, 0.4, 0),
                volmdlr.Vector3D(0, 0, 0.4),
            ),
            color=(0.2, 1, 0.4),
            alpha=0.6,
        )
        box_green = box_red.frame_mapping(
            volmdlr.Frame3D(
                volmdlr.Point3D(-0.4, 0, -0.1),
                volmdlr.Vector3D(1, 0, 0),
                volmdlr.Vector3D(0, 1, 0),
                volmdlr.Vector3D(0, 0, 1),
            ),
            "new",
        )
        ...
        box_blue = primitives3d.Block(
            volmdlr.Frame3D(
                volmdlr.Point3D(0.1, 0, 0),
                volmdlr.Vector3D(0.2, 0, 0),
                volmdlr.Vector3D(0, 0.1, 0),
                volmdlr.Vector3D(0, 0, 1),
            ),
            alpha=0.6,
        )
        box_blue2 = box_blue.frame_mapping(
            volmdlr.Frame3D(
                volmdlr.Point3D(0.2, 0, 0),
                volmdlr.Vector3D(1, 0, 0),
                volmdlr.Vector3D(0, 1.8, 0),
                volmdlr.Vector3D(0, 0, 1),
            ),
            "old",
        )
    ...
    def test_union_two_disjoint_objects(self):
        ...
        shell_faces = [
            faces.Triangle3D(*points) for points in poly1_vol1.sewing(poly2_vol1, volmdlr.X3D, volmdlr.Y3D)
        ] + [faces.Triangle3D(*points) for points in poly2_vol1.sewing(poly3_vol1, volmdlr.X3D, volmdlr.Y3D)]
