...
class TestBSplineCurve(unittest.TestCase):
    ...
    def test_line_intersections(self):
        ...
        line = vme.Line2D(
            volmdlr.Point2D(1.263163105753452, -0.002645572020392778),
    ...
    def test_discretization_points(self):
        control_points_2d = [
            volmdlr.Point2D(1.5707963267948966, 2.3),
        ...
        bspline_curve2d = vme.BSplineCurve2D(
            3,
            control_points_2d,
            [4, 1, 1, 1, 1, 1, 1, 1, 4],
            [
...
class TestBSplineCurve3D(unittest.TestCase):
    b_splinecurve3d = vme.BSplineCurve3D(
        degree=5,
        control_points=[
            volmdlr.Point3D(0.5334, 4.61e-10, -2.266),
            volmdlr.Point3D(0.5334, 0.236642912449, -2.26599999893),
            volmdlr.Point3D(0.5334, 0.473285829931, -2.23144925183),
            volmdlr.Point3D(0.5334, 0.70316976404, -2.16234807551),
            volmdlr.Point3D(0.5334, 1.13611540546, -1.95904362568),
            volmdlr.Point3D(0.5334, 1.49286052971, -1.64044168585),
            volmdlr.Point3D(0.5334, 1.64654439419, -1.45604332404),
            volmdlr.Point3D(0.5334, 1.77109261028, -1.25188280667),
            volmdlr.Point3D(0.5334, 1.86385510975, -1.03417888209),
        ],
    ...
    def test_line_intersections(self):
        line = vme.Line3D(
            volmdlr.Point3D(0.5334, -0.44659009801843536, 0.0),
    ...
    def test_linesegment_intersection(self):
        linesegment1 = vme.LineSegment3D(
            volmdlr.Point3D(0.5334, -0.44659009801843536, 0.0),
            volmdlr.Point3D(0.5334, 0.4342689853571558, -0.47337857496375274),
        )
        linesegment2 = vme.LineSegment3D(
            volmdlr.Point3D(0.5334, -0.44659009801843536, 0.0),
        ...
        self.assertEqual(
            bspline_lineseg_intersections2, [volmdlr.Point3D(0.5334, 1.784620481894723, -1.1990650295776075)]
        )
