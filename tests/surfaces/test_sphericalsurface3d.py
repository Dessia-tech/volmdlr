...
class TestSphericalSurface3D(unittest.TestCase):
    ...
    def test_contour3d_to_2d(self):
        surface = surfaces.SphericalSurface3D.from_json("surfaces/objects_spherical_tests/sphericalsurface1.json")
        contour = wires.Contour3D.from_json("surfaces/objects_spherical_tests/spericalsurface1_contour0.json")
        ...
        contour_left_side = wires.Contour3D.from_json("surfaces/objects_spherical_tests/contour_left_side.json")
        ...
        contour_rigth_side = wires.Contour3D.from_json("surfaces/objects_spherical_tests/contour_rigth_side.json")
        test = self.surface3d.contour3d_to_2d(contour_rigth_side)
        theta_min, theta_max, _, _ = test.bounding_rectangle.bounds()
        self.assertEqual(theta_min, 0)
        self.assertEqual(theta_max, math.pi)
        contour_left_side = wires.Contour3D.from_json("surfaces/objects_spherical_tests/contour_upper_side.json")
        test = self.surface3d.contour3d_to_2d(contour_left_side)
        _, _, phi_min, phi_max = test.bounding_rectangle.bounds()
        self.assertEqual(phi_min, 0)
        self.assertEqual(phi_max, 0.5 * math.pi)
        contour_rigth_side = wires.Contour3D.from_json("surfaces/objects_spherical_tests/contour_lower_side.json")
        test = self.surface3d.contour3d_to_2d(contour_rigth_side)
        _, _, phi_min, phi_max = test.bounding_rectangle.bounds()
