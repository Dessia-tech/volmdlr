...
x1, y1, z1 = (
    random.randrange(posmin, posmax, 1) / 100,
    random.randrange(posmin, posmax, 1) / 100,
    random.randrange(posmin, posmax, 1) / 100,
)
x2, y2, z2 = (
...
r1, r2 = (
    random.randrange(rmin / 10, rmax / 10, 1) / 1000,
    random.randrange(rmin / 2, rmax / 2, 1) / 1000,
)  # Radius of the arc3d generated
...
n2.Normalize()
plane1, plane2 = volmdlr.Plane3D.from_normal(c1, n1), volmdlr.Plane3D.from_normal(
    c2, n2
)  # Create a plane to give us two others vector
...
offset_theta1 = (
    random.randrange(angle_min, angle_max, 20) / 100
)  # Theta's offset if you want to turn it with normal's reference
...
pt1, pt2, pt3, pt4 = (
    volmdlr.Point2D((offset_theta1, offset_phi1)),
    volmdlr.Point2D((offset_theta1, offset_phi1 + phi1)),
    volmdlr.Point2D((offset_theta1 + theta1, offset_phi1 + phi1)),
    volmdlr.Point2D((offset_theta1 + theta1, offset_phi1)),
)
seg1, seg2, seg3, seg4 = (
    volmdlr.LineSegment2D(pt1, pt2),
    volmdlr.LineSegment2D(pt2, pt3),
    volmdlr.LineSegment2D(pt3, pt4),
    volmdlr.LineSegment2D(pt4, pt1),
)
...
segbh2 = volmdlr.LineSegment2D(
    center2d2, center2d2 + volmdlr.Point2D((0, h2)) + volmdlr.Point2D((angle_cyl / 3, 0))
)  #### Minus Pt2D because of Step adaptation
circlestart2 = volmdlr.LineSegment2D(
    segbh2.points[1], segbh2.points[1] + volmdlr.Point2D((angle_cyl, 0)) - volmdlr.Point2D((0, h2 / 10))
)  # You can change 2*pi by an other angle
seghb2 = volmdlr.LineSegment2D(
    circlestart2.points[1], circlestart2.points[1] - segbh2.points[1] + volmdlr.Point2D((angle_cyl / 3, 0))
)
