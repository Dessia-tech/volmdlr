...
p2 = vm.Point2D(0.0, 0.1755)
p3 = vm.Point2D(0.0, 0.04)
p4 = vm.Point2D(0.01, 0.04)
p5 = vm.Point2D(0.01, 0.0)
...
pbi2 = vm.Point2D(-B / 2.0, d1 / 2.0)
pbi1 = pbi2.translation(vm.Vector2D(h, 0))
pbi3 = vm.Point2D(-B / 2.0, d / 2.0)
pbi4 = vm.Point2D(B / 2.0, d / 2.0)
pbi5 = vm.Point2D(B / 2.0, d1 / 2.0)
...
z = vm.X3D.cross(y)
profile5 = RevolvedProfile(0.15 * vm.Y3D, vm.X3D, z, c5, 0.15 * vm.Y3D, vm.X3D, angle=0.7, name="strange part")
...
rim_contour = vmw.ClosedPolygon2D(
    [
        vm.Point2D(-0.5 * w, Rb),
...
rim_contour.plot()
conical_rim = RevolvedProfile(
    0.5 * vm.X3D, vm.X3D, z, rim_contour, 0.5 * vm.X3D, vm.X3D, angle=3.14, name="conical rim"
)
