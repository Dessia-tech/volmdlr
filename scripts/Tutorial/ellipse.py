...
extra1 = (
    Gradius * vm.Point3D(Gdir.vector) * math.cos(math.pi / 2)
    + Sradius * vm.Point3D(Sdir.vector) * math.sin(math.pi / 2)
    + center
)

segbh = vm.LineSegment3D(
    pointellipse, pointellipse + vm.Point3D([i * h for i in normal.vector])
