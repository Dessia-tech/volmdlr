...
for step in range(0, nb_step):
    ...
    for enum, polyfloor in enumerate(list_polyfloor[1:]):
        ...
        if with_borders:
            r = 0.15
            arc = vm.Arc2D(
                vm.Point2D((0, 0)), vm.Point2D(((-1 + math.sqrt(2) / 2) * r, r * math.sqrt(2) / 2)), vm.Point2D((-r, r))
            )
        ...
        if enum == len(list_polyfloor) - 2:
            if with_borders:
                ...
                arc_top = vm.Arc2D(
                    vm.Point2D((0, 0)),
                    vm.Point2D(((-1 + math.sqrt(2) / 2) * r_top, r_top * math.sqrt(2) / 2)),
                    vm.Point2D((-r_top, r_top)),
                )
                contour_top = vm.Contour2D([arc_top])
                contour_sweep_top = outer_contour_floor.Offset(r_top)

                new_wire_sweep = vm.Wire3D(
                    [
                        p.To3D(origin + extrusion_vector * (list_height_polyfloor[enum + 1]), x, y)
...
steps = [
    ...
    [vm.OXYZ] * (nb_components + 3 + len(primitives_floor[0]))
    + [away_frame] * (nb_primitives - nb_components - 3 - len(primitives_floor[0])),
    [vm.OXYZ] * (nb_components + 3 + len(primitives_floor[0]) + len(primitives_floor[1]))
