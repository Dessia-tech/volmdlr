...
shell1.alpha = 0.6
vm.model.VolumeModel([shell1]).babylonjs()

box_blue = primitives3D.Block(
    vm.Frame3D(vm.Point3D(0.1, 0, 0), vm.Vector3D(0.2, 0, 0),
               vm.Vector3D(0, 0.1, 0), vm.Vector3D(0, 0, 1)),
    alpha=0.6)
box_blue.color = (0.1, 0.1, 1)
box_blue.name = 'box_blue'

box_blue2 = box_blue.frame_mapping(vm.Frame3D(vm.Point3D(0.2, 0, 0), vm.Vector3D(1, 0, 0),
...
shell2.alpha = 0.6
vm.model.VolumeModel([shell2]).babylonjs()
