...
class LineSegment3D(LineSegment):
    ...
    def revolution(self, axis_point, axis, angle):
        ...
        line_inters = self.line_intersections(axis_line3d)
        if line_inters:
            if line_inters[0].point_distance(self.start) < line_inters[0].point_distance(self.end):
                axis_line3d = volmdlr_curves.Line3D(self.start, self.start + axis)
            else:
                axis_line3d = volmdlr_curves.Line3D(self.end, self.end + axis)
