...
def circle_3d_line_intersections(circle_3d, line):
    ...
    if line.point1.z == line.point2.z == circle_3d.frame.origin.z:
        ...
        quadratic_equation_b = (
            -2 * (direction_vector.y**2 / direction_vector.x**2) * line.point1.x
            + 2 * (direction_vector.y / direction_vector.x) * line.point1.y
        )
        quadratic_equation_c = (
            line.point1.y - (direction_vector.y / direction_vector.x) * line.point1.x
        ) ** 2 - circle_3d.radius**2
...
def ellipse2d_line_intersections(ellipse2d, line2d):
    ...
    if line2d.point2.x == line2d.point1.x:
        ...
        if ellipse2d.major_axis**2 * m**2 + ellipse2d.minor_axis**2 > c**2:
            x1 = -(
                2 * (ellipse2d.major_axis**2) * m * c
                + math.sqrt(
                    (2 * (ellipse2d.major_axis**2) * m * c) ** 2
                    - 4
                    * (ellipse2d.major_axis**2 * m**2 + ellipse2d.minor_axis**2)
                    * ellipse2d.major_axis**2
                    * (c**2 - ellipse2d.minor_axis**2)
                )
            ) / (2 * (ellipse2d.major_axis**2 * (m**2) + ellipse2d.minor_axis**2))
