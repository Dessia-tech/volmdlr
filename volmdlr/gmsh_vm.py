...
class GmshParser(DessiaObject):
    ...
    def from_file_entities(lines):
        ...
        return {"points": points_data, "curves": curves_data, "surfaces": surfaces_data, "volumes": volumes_data}
        ...
        while True:
            ...
            if num_nodes_in_block:
                ...
                for i in range(step, step + num_nodes_in_block):
                    line = lines[i].split()
                    points.append(volmdlr.mesh.Node3D(float(line[0]), float(line[1]), float(line[2])))
    ...
    def from_file_physical_names(lines):
        ...
        for i in range(1, int(lines[0].split()[0]) + 1):
            ...
            physical_names["physical_dim_" + physical_dim].append(
                {"physicalTag": int(lines[1][0]), "Name": lines[i][5:-1]}
            )
    ...
    def define_tetrahedron_element_mesh(self):
        ...
        for tetrahedrons in tetrahedron_elements:
            ...
            for tetrahedron in tetrahedrons:
                tetrahedrons_mesh.append(
                    volmdlr.mesh.TetrahedralElement(
                        [points[tetrahedron[0]], points[tetrahedron[1]], points[tetrahedron[2]], points[tetrahedron[3]]]
                    )
                )
        ...
        for triangles in triangles_elements:
            ...
            for triangle in triangles:
                if points[0].__class__.__name__[-2::] == "3D":
                    triangles_mesh.append(
                        volmdlr.mesh.TriangularElement3D(
                            [points[triangle[0]], points[triangle[1]], points[triangle[2]]]
                        )
                    )
                else:
                    triangles_mesh.append(
                        volmdlr.mesh.TriangularElement2D(
                            [points[triangle[0]], points[triangle[1]], points[triangle[2]]]
                        )
                    )
    ...
    def to_2d(list_nodes):
        ...
        return [volmdlr.mesh.Node2D(node[0], node[1]) for node in list_nodes]
    ...
    def get_lines_cells(self):
        ...
        cells_str_int = {"elements_type_1": ("2 ", 3), "elements_type_2": ("3 ", 4), "elements_type_4": ("4 ", 5)}
    ...
    def get_lines_cells_type(self):
        ...
        cells_str_int = {"elements_type_1": "3", "elements_type_2": "5", "elements_type_4": "10"}
