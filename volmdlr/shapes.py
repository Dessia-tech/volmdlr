...
class Shell(Shape):
    ...
    def __init__(self, faces, name: str = ''):
        self._faces = None
        if isinstance(faces, (TopoDS_Shell, TopoDS_Shape)):
            obj = faces
        elif isinstance(faces[0], vm_faces.Face3D):
            obj = None
            self._faces = faces
    ...
    def make_extrusion(cls, wire: wires.Contour3D, extrusion_direction: volmdlr.Vector3D,
        ...
        shell = BRepPrimAPI_MakePrism(ocp_wire, extrusion_vector)

        return cls(shell.Shape(), name=name)
...
class Solid(Shape):
    ...
    def __init__(self, obj: TopoDS_Solid, name: str = '') -> "Solid":
    ...
    def primitives(self):
        ...
        return [Shell(downcast(shape)) for shape in shape_set]

    @classmethod
    def make_extrusion(cls, face: vm_faces.PlaneFace3D,
                       extrusion_length: float, color: Tuple[float, float, float] = None,
                       alpha: float = 1., reference_path: str = volmdlr.PATH_ROOT, name: str = '') -> "Shell":
        """
        Returns a solid generated by the extrusion of a plane face.
    ...
    @classmethod
    def make_extrusion_from_wires(cls, frame: volmdlr.Frame3D,
        ...
        face = vm_faces.PlaneFace3D(surface3d=surfaces.Plane3D(frame),
                                    surface2d=surfaces.Surface2D(outer_contour2d, inner_contours2d)).to_ocp()
