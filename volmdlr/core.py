    ...
    def to_step_stream(self, stream: dcf.StringFile):
        ...
        for primitive in self.primitives:
            if primitive.__class__.__name__ in ('OpenShell3D', 'ClosedShell3D'):
                primitive_content, primitive_id = primitive.to_step_product(current_id)
            else:
                primitive_content, primitive_id, shape_representation_id = primitive.to_step(current_id)
