...
class Primitive2D(dc.DessiaObject):
    ...
    def generated_toroidalface(self, arcgen):
        ...
        toroidalsurface3d = ToroidalSurface3D(frame3d, rcenter * 1000,
                                              rcircle * 1000)

        theta = arcgen.angle
        phi = self.angle

        pt1, pt2, pt3, pt4 = Point2D((0, 0)), Point2D((0, phi)), Point2D(
            (theta, phi)), Point2D((theta, 0))
...
class Wire2D(CompositePrimitive2D):
    ...
    def plot_data(self, name='', fill=None, color='black', stroke_width=1,
                  opacity=1):
        plot_data = {}
        plot_data['name'] = name
        plot_data['type'] = 'wire'
...
class Contour2D(Wire2D):
    ...
    def plot_data(self,
        ...
        plot_data_primitives = [
            item.plot_data(plot_data_states=plot_data_states) for item in
            self.primitives]
        return plot_data.PlotDataContour2D(
...
class Mesh2D:
    ...
    def GeoScript(self, filepath=''):
        ...
        for contour in self.contours:
            contour_iprs = []
            for primitive in contour.primitives:
                spr, ipr2 = primitive.GeoScript(ipr, [points_index[p] for p in
                                                      primitive.geo_points])
                s += spr
                contour_iprs.extend(range(ipr, ipr2))
                ipr = ipr2
            s += 'Line Loop({}) = {{{}}};\n'.format(ipr,
                                                    str(contour_iprs)[1:-1])
            contours_indices.append(ipr)
            ipr += 1
        s += 'Plane Surface({}) = {{{}}};\n'.format(ipr, str(contours_indices)[
                                                         1:-1])
...
class LineSegment2D(Line2D):
    ...
    def dict_to_object(cls, dict_):
        return cls(point1=Point2D.dict_to_object(dict_['point1']),
                   point2=Point2D.dict_to_object(dict_['point2']),
                   name=dict_['name'])
    ...
    def PointAtCurvilinearAbscissa(self, curvilinear_abscissa):
        return self.points[0] + self.DirectionVector(
            unit=True) * curvilinear_abscissa
    ...
    def plot_data(self,
                  plot_data_states: List[plot_data.PlotDataState] = None):
        return plot_data.PlotDataLine2D(
    ...
    def CreateTangentCircle(self, point, other_line):
        ...
        if circle1 is not None:
            point_J1, curv_abs1 = Line2D.PointProjection(self, circle1.center,
                                                         True)
            if curv_abs1 < 0. or curv_abs1 > 1.:
                circle1 = None
        if circle2 is not None:
            point_J2, curv_abs2 = Line2D.PointProjection(self, circle2.center,
                                                         True)
...
class ScatterPlot(dc.DessiaObject):
    def __init__(self, nb_points_x: int, nb_points_y: int, font_size: int,
               graduation_color: str, axis_color: str, arrow_on:bool, name: str = ''):
...
class Tooltip(dc.DessiaObject):
    def __init__(self, colorfill:str, font:str, tp_width:float, tp_radius:float, to_plot_list:list, name: str = ''):
    ...
    def plot_data(self,
        ...
        if plot_data_states is None:
            plot_data_states = [plot_data.PlotDataState()]
        return plot_data.PlotDataTooltip(colorfill=self.colorfill, font=self.font, tp_width=self.tp_width, tp_radius=self.tp_radius, to_plot_list=self.to_plot_list, plot_data_states=plot_data_states, name=self.name)
...
class Plane3D(Primitive3D):
    ...
    def from_points(cls, points):
        if len(points) < 3:
            ...
            return cls.from_3_points(Point3D(points[0].vector),
                                     Vector3D(points[1].vector),
                                     Vector3D(points[2].vector))
        else:
    ...
    def Translation(self, offset, copy=True):
        if copy:
            ...
            return Plane3D(new_origin, self.vectors[0], self.vectors[1],
                           self.name)
        else:
...
class ToroidalSurface3D(Primitive3D):
    ...
    def frame_mapping(self, frame, side, copy=True):
        ...
        if side == 'new':
            ...
            new_frame = Frame3D(new_origin, new_u, new_v, new_w)
            if copy:
                return ToroidalSurface3D(new_frame, self.rcenter, self.rcircle,
                                         name=self.name)
            else:
                self.frame = new_frame
...
class Line3D(Primitive3D, Line):
    ...
    def __init__(self, point1, point2, name=''):
        Primitive3D.__init__(self, basis_primitives=[point1, point2],
                             name=name)
        self.points = [point1, point2]
    ...
    def PointAtCurvilinearAbscissa(self, curvilinear_abscissa):
        return self.points[0] + (
                self.points[1] - self.points[0]) * curvilinear_abscissa
...
class ArcEllipse3D(Primitive3D):
    ...
    def tessellation_points(self, resolution_for_ellipse=40):
        ...
        plane3d = Plane3D(self.center, self.major_dir, self.minor_dir,
                          self.normal)
...
class CompositePrimitive3D(Primitive3D):
    ...
    def __init__(self, primitives, name=''):
        ...
        Primitive3D.__init__(self, basis_primitives=basis_primitives,
                             name=name)
...
class Edge3D(Primitive3D):
    def __init__(self, edge_start, edge_end, name=''):
        Primitive3D.__init__(self, basis_primitives=[edge_start, edge_end],
                             name=name)
    ...
    def Rotation(self, center, axis, angle, copy=True):
        if copy:
            new_edge_start = self.points[0].Rotation(center, axis, angle,
                                                     copy=True)
            new_edge_end = self.points[1].Rotation(center, axis, angle,
                                                   copy=True)
    ...
    def frame_mapping(self, frame, side, copy=True):
        ...
        """
        if copy:
            new_edge_start = self.points[0].frame_mapping(frame, side,
                                                          copy=True)
            ...
            self.points[0] = self.points[0].frame_mapping(frame, side,
                                                          copy=True)
            self.points[1] = self.points[1].frame_mapping(frame, side,
                                                          copy=True)
...
class LineSegment3D(Edge3D):
    ...
    def dict_to_object(cls, dict_):
        return cls(point1=Point3D.dict_to_object(dict_['point1']),
                   point2=Point3D.dict_to_object(dict_['point2']),
                   name=dict_['name'])
    ...
    def PointAtCurvilinearAbscissa(self, curvilinear_abscissa):
        return self.points[0] + curvilinear_abscissa * (
                self.points[1] - self.points[0]) / self.Length()
...
class Contour3D(Wire3D):
    ...
    def __hash__(self):
        return sum([hash(e) for e in self.edges]) + sum(
...
class Circle3D(Contour3D):
    ...
    def __hash__(self):
        return int(round(1e6 * (self.center.vector[0] + self.center.vector[1] +
                                self.center.vector[2] + self.radius)))
    ...
    def __eq__(self, other_circle):
        return math.isclose(self.center.vector[0],
               ...
               and math.isclose(self.center.vector[1],
                                other_circle.center.vector[1], abs_tol=1e-06) \
               and math.isclose(self.center.vector[2],
                                other_circle.center.vector[2], abs_tol=1e-06) \
               and math.isclose(self.radius, other_circle.radius,
                                abs_tol=1e-06)
    ...
    def tessellation_points(self, resolution=20):
        ...
        tessellation_points_3D = [self.center + self.radius * math.cos(teta) *
                                  plane.vectors[0] + self.radius * math.sin(
            teta) * plane.vectors[1] \
                                  for teta in npy.linspace(0, 2 * math.pi,
                                                           resolution + 1)][
                                 :-1]
        return tessellation_points_3D
...
class Face3D(Primitive3D):
    ...
    def from_step(cls, arguments, object_dict):
        ...
        if object_dict[int(arguments[2])].__class__ is Plane3D:
            ...
            return ToroidalFace3D.from_contour3d(contours, object_dict[
                int(arguments[2])], name=arguments[0][1:-1])

        elif object_dict[int(arguments[2])].__class__ is ConicalSurface3D:
            return ConicalFace3D.from_contour3d(contours,
    ...
    def cut_contours(self, contours2d, resolution):
        ...
        segmentspt = []
        offset = 0.0001
        xmin, xmax = min([pt[0] for pt in placement2d]), max(
            [pt[0] for pt in placement2d])
        ymin, ymax = min([pt[1] for pt in placement2d]), max(
            [pt[1] for pt in placement2d])
        ...
        pointsxzmin = [Point2D([xmin + i * pas, ymin - 1]) for i in
                       range(0, resolution + 1)]
        pointsxzmax = [Point2D([xmin + i * pas, ymax + 1]) for i in
                       range(0, resolution + 1)]

        line = [LineSegment2D(ptxmax, ptxmin) for ptxmin, ptxmax in
                list(zip(pointsxzmin, pointsxzmax))]
...
class PlaneFace3D(Face3D):
    ...
    def __init__(self, contours, plane, points=None, polygon2D=None, name=''):
        ...
        self.contours = contours
        self.plane = plane
        self.setup_planeface(self.contours, self.plane, points=points,
                             polygon2D=polygon2D, name=self.name)

    def setup_planeface(self, ctrs2d, plane, points=None, polygon2D=None,
                        name=''):
        ...
        Face3D.__init__(self, [
            ctr3d.To3D(self.plane.origin, self.plane.vectors[0],
    ...
    def Rotation(self, center, axis, angle, copy=True):
        if copy:
            new_contour = [subcontour.Rotation(center, axis, angle, copy=True)
                           for subcontour in self.contour]
            new_plane = self.plane.Rotation(center, axis, angle, copy=True)
            new_points = [p.Rotation(center, axis, angle, copy=True) for p in
                          self.points]
            return (
                new_contour, new_plane, new_points, self.polygon2D, self.name)
        else:
            for contour in self.contours:
    ...
    def Translation(self, offset, copy=True):
        if copy:
            new_contour = [subcontour.Translation(offset, copy=True) for
                           subcontour in self.contours]
            new_plane = self.plane.Translation(offset, copy=True)
            new_points = [p.Translation(offset, copy=True) for p in
                          self.points]
            return (
                new_contour, new_plane, new_points, self.polygon2D, self.name)
        else:
    ...
    def triangulation(self):
        ...
        for i, contour in enumerate(self.contours):
            ...
            segments += [[a + total_len, a + total_len + 1] for a in
                         range(len_points - 1)] + [
                            [len_points + total_len - 1, 0 + total_len]]
            total_len += len_points
            points_3D.extend([pt.To3D(self.plane.origin, self.plane.vectors[0],
                                      self.plane.vectors[1]) for pt in
                              points_2D])
            if i > 0:
                ...
                if not polygon2D.PointBelongs(mid_point_2D):
                    warnings.warn(
                        'average_center_point is not included inside its contour.')
    ...
    def distance_to_face(self, face2, return_points=False):
        ...
        polygon1_points_3D = [Point3D(p.vector) for p in
                              self.contours3d[0].tessel_points]
        polygon2_points_3D = [Point3D(p.vector) for p in
...
class CylindricalFace3D(Face3D):
    ...
    def __init__(self, contours2d, cylindricalsurface3d, points=None, name=''):
        ...
        edge1 = Circle3D(cylindricalsurface3d.frame.origin, self.radius,
                         cylindricalsurface3d.frame.w)
        edge2 = Circle3D(cylindricalsurface3d.frame.origin +
                         contours2d[0].primitives[0].points[1][
                             1] * cylindricalsurface3d.frame.w, self.radius,
                         cylindricalsurface3d.frame.w)
    ...
    def from_contour3d(cls, contours3d, cylindricalsurface3d, name=''):
        ...
        if contours3d[0].edges[0].__class__ is LineSegment3D and \
        ...
        if contours3d[0].edges[0].__class__ is Arc3D and contours3d[0].edges[
            ...
            if not (
                    math.isclose(arc1.angle, abs(theta1_1 - theta1_2),
                                 abs_tol=1e-4)):
            ...
            pt1, pt2, pt3, pt4 = Point2D((offset1, hmin)), Point2D(
                (offset1, hmax)), Point2D((offset1 + angle1, hmax)), Point2D(
                (offset1 + angle1, hmin))
            seg1, seg2, seg3, seg4 = LineSegment2D(pt1, pt2), LineSegment2D(
                pt2, pt3), LineSegment2D(pt3, pt4), LineSegment2D(pt4, pt1)
    ...
    def from_arc3d(cls, lineseg, arc,
        ...
        segbh = LineSegment2D(Point2D((offset, 0)),
                              Point2D((offset, lineseg.Length())))
        circlestart = LineSegment2D(segbh.points[1],
                                    segbh.points[1] + Point2D((theta, 0)))
        seghb = LineSegment2D(circlestart.points[1],
    ...
    def points2d_to3d(self, all_contours_points, radius, frame3d):
        ...
        for listpt in all_contours_points:
            for enum, pt in enumerate(listpt):
                Points3D.append(Point3D(Vector3D(
                    [radius * math.cos(pt[0]), radius * math.sin(pt[0]),
                     pt[1]])))
    ...
    def contours3d_to2d(contours3d, cylindricalsurface3d):
            ...
            if edge.__class__ is Arc3D:
                if edge.normal == n or edge.normal == -n:
                    ...
                    if math.isclose(edge.angle, 2 * math.pi, abs_tol=1e-6):
                        if start2d == end2d:
                            if math.isclose(start2d.vector[0], 2 * math.pi,
                                            abs_tol=1e-6):
                    ...
                    for k in range(0, len(points2d) - 1):
                        lines.append(
                            LineSegment2D(points2d[k], points2d[k + 1]))
                    points, prim_list = [], []
        ...
        pt1, pt2, pt3, pt4 = Point2D((xmin, ymin)), Point2D(
            (xmin, ymax)), Point2D((xmax, ymin)), Point2D((xmax, ymax))
                    ...
                    if math.isclose(intersect, math.pi, abs_tol=5e-2):
                        ...
                        for pt in points_sing:
                            if math.isclose(pt.vector[0], 0, abs_tol=1e-2):
                                pt0 += 1
                            elif math.isclose(pt.vector[0], 2 * math.pi,
                                              abs_tol=1e-2):
                                pt2pi += 1
                                ...
                                for pt in prim.points:
                                    if pt[0] < intersect:
                                        ...
                                        if math.isclose(pt[0], 0,
                                                        abs_tol=1e-1):
                                            new_list_points.append(Point2D((
                                                intersect + 2 * math.pi,
                                                pt[
                                                    1])))
                                        else:
                        ...
                        for pt in prim.points:
                            if pt[0] < x_intersect:
                                ...
                                if math.isclose(pt[0], 0, abs_tol=1e-1):
                                    new_list_points.append(Point2D(
                                        (x_intersect + 2 * math.pi, pt[1])))
                                else:
                                    new_list_points.append(
                                        Point2D((2 * math.pi + pt[0], pt[1])))
                            else:
                                new_list_points.append(pt)
                        if change > 0:
                            points.extend(new_list_points)
                            primitives[k] = LineSegment2D(new_list_points[0],
                                                          new_list_points[1])
                        else:
                            points.extend(prim.points)
                            continue
    ...
    def minimum_distance_points_cyl(self, other_cyl):
        ...
        res = [res2, res3]
        for couple in res:
            pttest1 = Point3D((r1 * math.cos(couple.x[0]),
                               r1 * math.sin(couple.x[0]), couple.x[1]))
            pttest2 = Point3D((r2 * math.cos(couple.x[2]),
        ...
        pt1_2d, pt2_2d = Point2D((result.x[0], result.x[1])), Point2D(
...
class ToroidalFace3D(Face3D):
    ...
    def __init__(self, contours2d, toroidalsurface3d, param, name=''):
        ...
        c1 = Arc3D(ptext, self.center + Point3D((
                                                        self.rcenter * vec1 * math.cos(
                                                    param[
                                                        0] / 2) + self.rcenter * vec2 * math.sin(
                                                    param[0] / 2)).vector),
                   self.center + Point3D((self.rcenter * vec1 * math.cos(
                       param[0]) + self.rcenter * vec2 * math.sin(
                       param[0])).vector), self.normal)
    ...
    def from_contour3d(cls, contours3d, toroidalsurface3d, name=''):
        ...
        if contours3d[0].__class__ is Point3D:  # If it is a complete tore
            angle = 2 * math.pi
            pt1, pt2, pt3, pt4 = Point2D((0, 0)), Point2D((0, angle)), Point2D(
                (angle, angle)), Point2D((angle, 0))
            seg1, seg2, seg3, seg4 = LineSegment2D(pt1, pt2), LineSegment2D(
                pt2, pt3), LineSegment2D(pt3, pt4), LineSegment2D(pt4, pt1)
            primitives = [seg1, seg2, seg3, seg4]
            contours2d = [Contour2D(primitives)]
            ...
            1].__class__ is Arc3D:  # Portion of Tore
            ...
            pt1, pt2, pt3, pt4 = Point2D((0, 0)), Point2D((0, phi1)), Point2D(
                (theta, phi2)), Point2D((theta, 0))
            seg1, seg2, seg3, seg4 = LineSegment2D(pt1, pt2), LineSegment2D(
                pt2, pt3), LineSegment2D(pt3, pt4), LineSegment2D(pt4, pt1)
            primitives = [seg1, seg2, seg3, seg4]
            contours2d = [Contour2D(primitives)]
            ...
            2].__class__ is Arc3D:  # if it is a Tore but not linked by Arc3D
            points2d = []
            for edge in contours3d[0].edges:
                new_points = [frame.NewCoordinates(pt) for pt in edge.points]
                points = ToroidalFace3D.points3d_to2d(new_points, rcenter,
                                                      rcircle)
            ...
            contours2d = ToroidalFace3D.contours3d_to2d(contours3d,
                                                        toroidalsurface3d)
            theta = max(pt[0] for pt in contours2d[0].tessel_points) - min(
                pt[0] for pt in contours2d[0].tessel_points)
            phi = max(pt[1] for pt in contours2d[0].tessel_points) - min(
    ...
    def from_arc3d(cls, arc, arcgen):
        ...
        if arc.__class__.__name__ == 'Circle3D' or arc.__class__.__name__ == 'Circle2D':
            ...
            if last_generated.__class__ is Point3D:
                ...
                last_generated = last_generated.To2D(center_generated, u_g,
                                                     v_g)
        ...
        pt1, pt2, pt3, pt4 = Point2D((offset1, offset2)), Point2D(
            ...
            (theta + offset1, phi + offset2)), Point2D(
            (theta + offset1, offset2))
        seg1, seg2, seg3, seg4 = LineSegment2D(pt1, pt2), LineSegment2D(pt2,
                                                                        pt3), LineSegment2D(
            pt3, pt4), LineSegment2D(pt4, pt1)
        edges = [seg1, seg2, seg3, seg4]
    ...
    def points3d_to2d(points3d, rcenter, rcircle):
        ...
        #     points_2D = [new_points2d[0], new_points2d[-1]]
            ...
            new_points2d = [pt.copy() for pt in points_2D]
            new_points2d.sort(key=lambda pt: pt[0])
            if i2pi == 2 or i0 == 2 or (i0 >= 1 and iangle == 2) or (
                    i2pi >= 1 and iangle == 2):
                pt1, pt2 = new_points2d[0], new_points2d[-1]
                pt1.vector[0] = 0 - 0.0000001
                pt2.vector[0] = 2 * math.pi + 0.0000001
    ...
    def contours3d_to2d(contours3d, toroidalsurface3d):
        ...
        for edge in contours3d[0].edges:
            ...
            if edge.__class__ is Arc3D:
                if edge.normal == n or edge.normal == -n:
                    start2d, end2d = ToroidalFace3D.points3d_to2d(new_points,
                                                                  rcenter,
                                                                  rcircle)
                    # if math.isclose(edge.angle, 2*math.pi, abs_tol=1e-6):
                    #     if start2d == end2d :
                    ...
                    points2d = ToroidalFace3D.points3d_to2d(new_points,
                                                            rcenter, rcircle)
                    lines = []
                    for k in range(0, len(points2d) - 1):
                        lines.append(
                ...
                points2d = ToroidalFace3D.points3d_to2d(new_points, rcenter,
                                                        rcircle)
                lines = []
                for k in range(0, len(points2d) - 1):
                    lines.append(LineSegment2D(points2d[k], points2d[k + 1]))
    ...
    def triangulation(self, resolution=30):
        ...
        for listpt in ctr_pt1:
            ...
            all_contours_points.extend(
                self.cut_contours(ctr2d, resolution_phi))
        ...
        all_points = self.delete_double(
            pts_frame1)  # All points necessary to triangulate
        all_points.sort(key=lambda pt: pt[0])
        ptvert, pts = [], []
        for k in range(0, resolution_theta + 1):
            ptvert.append([point for point in all_points[
                                              k * (resolution_phi + 1):(
                                                                               k + 1) * (
                                                                               resolution_phi + 1)]])
        ...
        Triangles, ts = [], []
        if math.isclose(angle_phi, 2 * math.pi, abs_tol=1e-6) or math.isclose(
                angle_theta, 2 * math.pi, abs_tol=1e-6):
            step = resolution_phi
            for k in range(0, len(pts) - resolution_phi - 2):
                ...
                listpt = [pts[k], pts[k + 1], pts[k + 1 + resolution_phi + 1],
                          pts[k + resolution_phi + 1]]
                # print('>>>>>>listpt', listpt)
                listindice = [k, k + 1, k + 1 + resolution_phi + 1,
                              k + resolution_phi + 1]
                for i, pt in enumerate(listpt):
                    vertices.append(pt.vector)
                    segments.append([i, i + 1])
    ...
    def minimum_distance_points_tore(self, other_tore):
        ...
        pt1_2d, pt2_2d = Point2D((result.x[1], result.x[0])), Point2D(
    ...
    def minimum_distance_points_cyl(self, cyl):
        ...
        res = [res2, res3]
        for couple in res:
            pttest1 = Point3D((r * math.cos(couple.x[0]),
...
class ConicalFace3D(Face3D):
    ...
    def __init__(self, contours2d, conicalsurface3d, param, name=''):
        ...
        ptext1 = self.center + Point3D((self.rb * vec1).vector) + self.normal * \
                 param[4]
        center2 = self.center + param[5] * self.normal
        ptext2 = center2 + Point3D((self.rt * vec1).vector) + param[
            5] * self.normal
        if math.isclose(self.rb, 0, abs_tol=1e-6):
            c1 = LineSegment3D(self.center, center2)
        else:
            c1 = Arc3D(ptext1,
                       ptext1.Rotation(self.center, self.normal, param[3] / 2),
                       ptext1.Rotation(self.center, self.normal, param[3]),
                       self.normal)
        if math.isclose(self.rt, 0, abs_tol=1e-6):
            c2 = LineSegment3D(center2, self.center)
        else:
            c2 = Arc3D(ptext2,
                       ptext2.Rotation(center2, self.normal, param[1] / 2),
                       ptext2.Rotation(center2, self.normal, param[1]),
                       self.normal)
    ...
    def from_contour3d(cls, contours3d, conicalsurface3d, name=''):
        ...
        if contours3d[0].edges[0].__class__ is Arc3D and contours3d[0].edges[
            1].__class__ is LineSegment3D and size <= 4:  # Portion of Cone

            hmax, hmin, Rmax, Rmin = ConicalFace3D.from_points3d_param(
                contours3d[0].tessel_points, frame)
            if contours3d[0].edges[2].__class__ is LineSegment3D:
                radius_max = max([Rmax, Rmin])
                arc = contours3d[0].edges[0]
                if arc.normal == -frame.w:
                    arc.setup_arc(arc.start, arc.interior, arc.end,
                                  -arc.normal)
                theta1, theta2 = posangle_arc(arc.start, arc.end, radius_max,
                                              frame)
                offset, angle = offset_angle(arc.is_trigo, theta1, theta2)
                pt1, pt2, pt3, pt4 = Point2D((offset, hmin)), Point2D(
                    (offset, hmax)), Point2D((offset + angle, hmax)), Point2D(
                    (offset + angle, hmin))
                ...
                if n1 == -frame.w:
                    arc1.setup_arc(arc1.start, arc1.interior, arc1.end,
                                   -arc1.normal)
                if n2 == -frame.w:
                    arc2.setup_arc(arc2.start, arc2.interior, arc2.end,
                                   -arc2.normal)
                start1, end1 = arc1.start, arc1.end
                start2, end2 = arc2.start, arc2.end
                if math.isclose(r1, Rmin, abs_tol=1e-6):
                    theta1_1, theta1_2 = posangle_arc(start1, end1, Rmin,
                                                      frame)
                    offset1, angle1 = offset_angle(arc1.is_trigo, theta1_1,
                                                   theta1_2)

                    theta2_1, theta2_2 = posangle_arc(start2, end2, Rmax,
                                                      frame)
                    offset2, angle2 = offset_angle(arc2.is_trigo, theta2_1,
                                                   theta2_2)

                    pt1, pt2, pt3, pt4 = Point2D((offset1, hmin)), Point2D(
                        (offset2, hmax)), Point2D(
                        (offset2 + angle2, hmax)), Point2D(
                        (offset1 + angle1, hmin))
                elif math.isclose(r1, Rmax, abs_tol=1e-6):
                    theta1_1, theta1_2 = posangle_arc(start2, end2, Rmin,
                                                      frame)
                    offset1, angle1 = offset_angle(arc2.is_trigo, theta1_1,
                                                   theta1_2)

                    theta2_1, theta2_2 = posangle_arc(start1, end1, Rmax,
                                                      frame)
                    offset2, angle2 = offset_angle(arc1.is_trigo, theta2_1,
                                                   theta2_2)
        ...
        elif contours3d[0].edges[1].__class__ is Arc3D and contours3d[0].edges[
            0].__class__ is LineSegment3D and size <= 4:
            hmax, hmin, Rmax, Rmin = ConicalFace3D.from_points3d_param(
                contours3d[0].tessel_points, frame)
            ...
            if size > 3:
                if contours3d[0].edges[3].__class__ is Arc3D:
                    ...
                    if arc.normal == -frame.w:
                        arc.setup_arc(arc.start, arc.interior, arc.end,
                                      -arc.normal)
                    theta1, theta2 = posangle_arc(arc.start, arc.end,
                                                  radius_max, frame)
                    offset, angle = offset_angle(arc.is_trigo, theta1, theta2)
                    pt1, pt2, pt3, pt4 = Point2D((offset, hmin)), Point2D(
                        (offset, hmax)), Point2D(
                        (offset + angle, hmax)), Point2D(
                        (offset + angle, hmin))
            else:
                radius_max = max([Rmax, Rmin])
                arc = contours3d[0].edges[1]
        ...
        elif contours3d[0].edges[0].__class__ is LineSegment3D and \
                contours3d[0].edges[
                    1].__class__ is LineSegment3D and size == 3:
            hmax, hmin, Rmax, Rmin = ConicalFace3D.from_points3d_param(
                contours3d[0].tessel_points, frame)
            ...
            if contours3d[0].edges[2].__class__ is Arc3D:
                ...
                pt1, pt2, pt3, pt4 = Point2D((offset, hmin)), Point2D(
                    (offset, hmax)), Point2D((offset + angle, hmax)), Point2D(
                    (offset + angle, hmin))
                seg1, seg2, seg3, seg4 = LineSegment2D(pt1,
                                                       pt2), LineSegment2D(pt2,
                                                                           pt3), LineSegment2D(
                    pt3, pt4), LineSegment2D(pt4, pt1)
                primitives = [seg1, seg2, seg3, seg4]
                contours2d = [Contour2D(primitives)]
                ...
                contours2d = ConicalFace3D.contours3d_to2d(contours3d,
                                                           conicalsurface3d)
                theta = max(pt[0] for pt in contours2d[0].tessel_points) - min(
                    pt[0] for pt in contours2d[0].tessel_points)
                param = [Rmin, angle, Rmax, angle, hmin, hmax]
            ...
            # Creation of the window
            pt1, pt2, pt3, pt4 = Point2D((offset, hmin)), Point2D(
                (offset, hmax)), Point2D((offset + theta2, hmax)), Point2D(
                (theta1, hmin))
            seg1, seg2, seg3, seg4 = LineSegment2D(pt1, pt2), LineSegment2D(
                pt2, pt3), LineSegment2D(pt3, pt4), LineSegment2D(pt4, pt1)
            primitives = [seg1, seg2, seg3, seg4]
    ...
    def points2d_to3d(self, all_contours_points, rb, rt, hmin, hmax, frame3d):
        ...
        for listpt in all_contours_points:
            for pt in listpt:
                ...
                Points3D.append(Point3D((radius * math.cos(pt[0]),
                                         radius * math.sin(pt[0]), pt[1])))
        Points_3D = [frame3d.OldCoordinates(point) for point in Points3D]
        return Points_3D
    ...
    def contours3d_to2d(contours3d, conicalsurface3d):
        ...
        for edge in contours3d[0].edges:
            ...
            if edge.__class__ is Arc3D:
                if edge.normal == n or edge.normal == -n:
                    start2d, end2d = ConicalFace3D.points3d_to2d(new_points,
                                                                 edge.radius,
                                                                 edge.radius,
                                                                 hmin, hmax)
                    angle2d = abs(end2d[0] - start2d[0])
                    ...
                    points2d = ConicalFace3D.points3d_to2d(new_points,
                                                           edge.radius,
                                                           edge.radius, hmin,
                                                           hmax)
                    lines = []
                ...
                start2d, end2d = ConicalFace3D.points3d_to2d(new_points, Rmin,
                                                             Rmax, hmin, hmax)
                ls_toadd = LineSegment2D(start2d, end2d)
                same = Face3D.LS2D_inprimitives(ls_toadd, primitives)
                if same is False:
                ...
                points2d = ConicalFace3D.points3d_to2d(new_points, Rmin, Rmax,
                                                       hmin, hmax)
                lines = []
                for k in range(0, len(points2d) - 1):
                    lines.append(LineSegment2D(points2d[k], points2d[k + 1]))
                    ...
                    if math.isclose(intersect, math.pi, abs_tol=5e-2):
                        ...
                        for list_prim in primitives:
                            for k, prim in enumerate(list_prim):
                                ...
                                if change > 0:
                                    points.extend(new_list_points)
                                    list_prim[k] = LineSegment2D(
                                        new_list_points[0], new_list_points[1])
                                else:
                                    points.extend(prim.points)
                                    continue
                        points_cleaned = delete_double_point(points)
    ...
    def triangulation(self, resolution=30):
        ...
        resolution_theta = abs(
            int(max(angle_theta1, angle_theta2) / pas_theta))
        if resolution_theta < 5:
            resolution_theta = 5

        all_contours_points = self.cut_contours(self.contours2d,
                                                resolution_theta)
        ...
        if self.rb == 0:
            # print('>>>>>>>1')
            points, Triangles = self.create_triangle(all_contours_points,
                                                     part='bot')
            Points_3D = self.points2d_to3d([points], self.rb, self.rt, hmin,
                                           hmax, frame3d)
        elif self.rt == 0:
            # print('>>>>>>>2')
            points, Triangles = self.create_triangle(all_contours_points,
            ...
            Points_3D = self.points2d_to3d(all_contours_points, self.rb,
                                           self.rt, hmin, hmax, frame3d)
    ...
    def frame_mapping(self, frame, side, copy=True):
        if copy:
            new_conicalsurface3d = ConicalSurface3D.frame_mapping(frame, side,
                                                                  copy)
            return ConicalFace3D(self.contours2d, new_conicalsurface3d,
                                 points=self.points, name=self.name)
        else:
...
class SphericalFace3D(Face3D):
    ...
    def __init__(self, contours2d, sphericalsurface3d, points=None, name=''):
        ...
        c1 = Arc3D(ptext1,
                   ptext1.Rotation(self.center, self.normal, points[0] / 2),
                   ptext1.Rotation(self.center, self.normal, points[0]),
                   self.normal)
        c2 = Arc3D(ptext2,
                   ptext2.Rotation(self.center, self.normal, points[1] / 2),
                   ptext2.Rotation(self.center, self.normal, points[1]),
                   self.normal)
    ...
    def from_contour3d(cls, contours3d, sphericalsurface3d, name=''):
        ...
        if contours3d[0].__class__ is Point3D:  # If it is a complete sphere
            ...
            # Creation of the window
            pt1, pt2, pt3, pt4 = Point2D((offset_theta, offset_phi)), Point2D(
                (offset_theta, offset_phi + phi)), Point2D(
                (offset_theta + theta, offset_phi + phi)), Point2D(
    ...
    def contours3d_to2d(contours3d, sphericalsurface3d):
        ...
        for edge in contours3d[0].edges:
            ...
            if edge.__class__ is Arc3D:
                if edge.normal == n or edge.normal == -n:
                    start2d, end2d = SphericalFace3D.points3d_to2d(new_points,
                                                                   radius)
                    angle2d = abs(end2d[0] - start2d[0])
                    if math.isclose(edge.angle, 2 * math.pi, abs_tol=1e-6):
                        if start2d == end2d:
                    ...
                    points2d = SphericalFace3D.points3d_to2d(new_points,
                                                             radius)
                    lines = []
                    for k in range(0, len(points2d) - 1):
                        lines.append(
...
class BSplineFace3D(Face3D):
    ...
    def _contour_bspline(self):
        if self.bspline_shape.__class__ is BSplineExtrusion:
            ...
            list_extrudeh = [normal.Dot(Vector3D(pt.vector)) for pt in
                             self.bspline_shape.points]
            # find the bd box's height ,changing the frame of points
            list_h = [normal.Dot(Vector3D(pt.vector)) for pt in
                      self.contours[0].tessel_points[:]]
            ...
            surface.knotvector_u = utilities.generate_knot_vector(
                surface.degree_u, surface.ctrlpts_size_u)
            surface.knotvector_v = utilities.generate_knot_vector(
                surface.degree_v, surface.ctrlpts_size_v)
            surface.delta = 0.05
            surface_points = surface.evalpts
            ...
            if len(self.contours[0].edges) == 4:
                ...
                for u, list_pointv in enumerate(list_pointsv):
                    for v, pt in enumerate(list_pointv):
                        ...
                        if pt_2d.vector[0] == 0 or pt_2d.vector[0] == 1 or \
                                pt_2d.vector[1] == 0 or pt_2d.vector[1] == 1:
                ...
                for pt2d in range_contours2d:
                    u, v = int(pt2d.vector[0] * (nbu - 1)), int(
                        pt2d.vector[1] * (nbv - 1))
                ...
                points_surface, points_contour = points, self.contours[
                    0].tessel_points
                list_pointsv, tessel_points2d = BSplineFace3D.localisation(nbu,
                                                                           nbv,
                                                                           points_surface,
                                                                           points_contour)
                ...
                minu, maxu = int(
                    min(pt[0] for pt in tessel_points2d) * (nbu - 1)), int(
                    max(pt[0] for pt in tessel_points2d) * (nbu - 1)) + 1
                minv, maxv = int(
                    min(pt[1] for pt in tessel_points2d) * (nbv - 1)), int(
                    max(pt[1] for pt in tessel_points2d) * (nbv - 1)) + 1
            ...
            nbu, nbv = int(1 / delta[0]) + 1, int(1 / delta[1]) + 1
            points_surface, points_contour = self.bspline_shape.points, \
                                             self.contours[0].tessel_points
            list_pointsv, tessel_points2d = BSplineFace3D.localisation(nbu,
                                                                       nbv,
                                                                       points_surface,
    ...
    def localisation(nbu, nbv, points_surface, points_contour):
        ...
        for point in points_contour:
            ...
            for list_pointv in list_pointsv:
                ...
                d = [point.point_distance(point1),
                     point.point_distance(point2)]
                posv = [0, 1]
            ...
            point_2d = (point1_2d * ((D - d1) / D) + point2_2d * (
                    (D - d2) / D) + point3_2d * (
                                (D - d3) / D) + point4_2d * (
                                (D - d4) / D)) / 3
...
class Shell3D(CompositePrimitive3D):
    ...
    def copy(self):
        ...
        return Shell3D(new_faces, alpha=self.alpha, color=self.color,
                       name=self.name)
...
class Step:
    ...
    def instanciate(self, instanciate_id, object_dict):
        ...
        elif name in step_to_volmdlr_primitive and hasattr(
                step_to_volmdlr_primitive[name], "from_step"):
            volmdlr_object = step_to_volmdlr_primitive[name].from_step(
...
step_to_volmdlr_primitive = {
    ...
    'SURFACE_OF_LINEAR_EXTRUSION': BSplineExtrusion,
