...
class Circle3D(Contour3D):
    ...
    def linesegment_intersections(self, linesegment: volmdlr.edges.LineSegment3D):
        ...
        if not math.isclose(abs(self.frame.w.dot(volmdlr.Z3D)), 1, abs_tol=1e-6):
            frame_mapped_circle = self.frame_mapping(self.frame, 'new')
            frame_mapped_lineseg = linesegment.frame_mapping(self.frame, 'new')
            ...
            for inter in circle_linseg_intersections:
                intersections.append(self.frame.old_coordinates(inter))
            return intersections
