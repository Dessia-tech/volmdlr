...
class Wire:
    ...
    def extract_without_primitives(self, point1, point2):
        ...
        return self.extract_primitives(point1, split_primitives[0],
                                       point2, split_primitives[1])
...
class Wire2D(volmdlr.core.CompositePrimitive2D, Wire):
    ...
    def extract(self, point1, primitive1, point2, primitive2):
        return Wire2D(self.extract_primitives(point1, primitive1,
                                              point2, primitive2))
...
class Contour:
    ...
    def extract_primitives(self, point1: Union[volmdlr.Point2D,
                                               volmdlr.Point3D],
                           primitive1: Union[volmdlr.core.Primitive2D,
                                             volmdlr.core.Primitive3D],
                           point2: Union[volmdlr.Point2D,
                                         volmdlr.Point3D],
                           primitive2: Union[volmdlr.core.Primitive2D,
                                             volmdlr.core.Primitive3D]):
        primitives = []
        ip1 = self.primitive_to_index(primitive1)
        ip2 = self.primitive_to_index(primitive2)
...
class ClosedPolygon2D(Contour2D, ClosedPolygon):
    ...
    def polygon_from_segments(cls, list_point_pairs):
        ...
        list_point_pairs.remove((list_point_pairs[0][0],
                                 list_point_pairs[0][1]))
        finished = False
    ...
    def is_trigo(self):
        ...
        angle = 0.
        for ls1, ls2 in zip(self.line_segments, self.line_segments[1:]
    ...
    def concave_hull(cls, points, concavity, scale_factor):
        ...
        return cls.polygon_from_segments([(line.start, line.end) for line in
                                          hull_concave_edges])  # , nearby_points
...
class ClosedPolygon3D(Contour3D, ClosedPolygon):
    ...
    def sewing(self, polygon2):
        center1, center2 = self.average_center_point(),\
            polygon2.average_center_point()
        center1, center2 = volmdlr.Point3D(center1.x, center1.y,
                                           0), volmdlr.Point3D(center2.x,
                                                               center2.y, 0)
        new_polygon1, new_polygon2 = self.translation(
            -center1), polygon2.translation(-center2)
        ...
        for i, point_polygon1 in enumerate(
            ...
            if i != 0:
                mean_point = 0.5 * (
                        point_polygon1 + new_polygon1.points[i - 1])
                distances = [mean_point.point_distance(point_poly2) for
                             point_poly2 in new_polygon2.points]
                closing_point = new_polygon2.points[
                ...
                while not_valid_point:
                    ...
                    if closing_point in list_closing_points:
                        ...
                        if closing_point != list_closing_points[-1]\
                                and closing_point != list_closing_points[0]:
                            distances.remove(min(distances))
                            closing_point = new_polygon2.points[
                                distances.index(min(distances))]
                            if new_polygon2.points.index(closing_point)\
                                    < new_polygon2.points.index(
                                    list_closing_points[-1]):
                                # print("yes")
                                distances.remove(min(distances))
                                closing_point = new_polygon2.points[
                                    distances.index(min(distances))]
                ...
                real_closing_point = polygon2.points[
                    distances.index(min(distances))]
                if i == 1:
                ...
                if closing_point != previous_closing_point:
                    dict_closing_pairs[self.points[i - 1]] = (
                        new_polygon2.points.index(previous_closing_point),
                    ...
                    if i - 1 >= index[0] and i <= index[1]:
                        ...
                        if index[0] > index[1]:
                            if ((i - 1 <= index[0] and i <= index[1]) or (
                                ...
                                triangles.append(
                                    [polygon2.points[i - 1],
                                     polygon2.points[new_polygon2.points.index(
                                                     point_polygon2)],
                                     list(dict_closing_pairs.keys())[j]])
