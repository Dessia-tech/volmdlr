...
class Contour:
    ...
    def contours_from_edges(cls, edges):
        ...
        contour = []
        counter = 0
        print("edges points :", [(prim.start, prim.end) for prim in edges])
        while not finished:
            len1 = len(edges)
            for line in edges:
            ...
            if len(edges) == 1:
                counter += 1
            if counter == 1:
                raise ValueError("problem in generating contour from edges")
            print("len(edges) :", len(edges))
            print("counter :", counter)
        print("list_contours :", list_contours)
            ...
            for j, index in enumerate(list(dict_closing_pairs.values())):
                if i != 0:
                    if i - 1 >= index[0] and i <= index[1]:
                        ...
                        if (
                                a != b and a != c and b != c and not volmdlr.edges.LineSegment3D(
                            a, c).point_belongs(b)):
